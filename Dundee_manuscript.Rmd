...
#BE_clr <- as.data.frame(clr(BE_t))

#mds <- metaMDS(BE_clr, distance = "bray", k = 3, trymax = 100, autotransform = TRUE)
#clr results in negative values, not valid for NMDS
...
#possible brewer.pal: GnBu, PuBu, PuBuGn
...
scores_signif$species <- c("S. parasanguinis", "R. mucilaginosa", "H. influenzae", 
                           "P. melaninogenica", "V. atypica", "S. salivarius", "P. aeruginosa",
                           "N. subflava", "G. sanguinis", "S. equinus", "V. dispar",
                           "P. jejuni", "S. odontolytica", "H. parainfluenzae", "E. sulci",
                           "F. periodonticum", "A. parvulum", "N. mucosa", "P. denticola",
...
#line plots species distribution
```{r}

BE_qualSpecies <- BE_t

BE_qualSpecies00 <- as.data.frame(colSums(BE_qualSpecies>0.01))
colnames(BE_qualSpecies00) <- c("Bronchiectasis")


healthy_qualSpecies <- healthy_t

healthy_qualSpecies00 <- as.data.frame(colSums(healthy_qualSpecies>0.01))
colnames(healthy_qualSpecies00) <- c("Healthy")

#combine

qualSpecies <- merge(BE_qualSpecies00, healthy_qualSpecies00, by = 0, all = TRUE)

rownames(qualSpecies) <- qualSpecies$Row.names
qualSpecies$Row.names <- NULL
qualSpecies[is.na(qualSpecies)] <- 0

qualSpecies00 <- qualSpecies[as.logical(rowSums(qualSpecies != 0)),]
qualSpecies00 <- qualSpecies00[order(-qualSpecies00$Healthy),]
qualSpecies00$species <- rownames(qualSpecies00)
#116 species


level_order <- qualSpecies00$species

qualSpecies_long <- gather(qualSpecies00, key = "state", value = "count",
                                -c(species))


ggplot(qualSpecies_long, aes(x = reorder(species, -count), 
                             y = count, color = state, group = state)) +
  geom_line() +
  geom_point() +
  theme_pubr()

qualSpecies_long$speciesOrder <- factor(qualSpecies_long$species,
                                        levels = level_order)

ggplot(qualSpecies_long, aes(x = speciesOrder, 
                             y = count, color = state, group = state)) +
  geom_line() +
  geom_point() +
  coord_flip() +
...
BE_qualSpecies10 <- as.data.frame(colSums(BE_qualSpecies>0.05))
colnames(BE_qualSpecies10) <- c("Bronchiectasis")

#change absolute counts to percent abundance
BE_qualSpecies100 <- as.data.frame(BE_qualSpecies10/101)


healthy_qualSpecies <- healthy_t

healthy_qualSpecies10 <- as.data.frame(colSums(healthy_qualSpecies>0.05))
colnames(healthy_qualSpecies10) <- c("Healthy")

#change absolute counts to percent abundance
healthy_qualSpecies100 <- as.data.frame(healthy_qualSpecies10/97)

#combine

qualSpecies10 <- merge(BE_qualSpecies100, healthy_qualSpecies100, by = 0, all = TRUE)
#

rownames(qualSpecies10) <- qualSpecies10$Row.names
qualSpecies10$Row.names <- NULL
qualSpecies10[is.na(qualSpecies10)] <- 0

qualSpecies11 <- qualSpecies10[as.logical(rowSums(qualSpecies10 != 0)),]
qualSpecies11 <- qualSpecies11[order(-qualSpecies11$Healthy),]
qualSpecies11$species <- rownames(qualSpecies11)

qualSpecies11$species <- gsub(x = qualSpecies11$species, 
                                      pattern = "\\_", replacement = " ")



level_order11 <- qualSpecies11$species

qualSpecies11_long <- gather(qualSpecies11, key = "state", value = "count",
                                -c(species))


ggplot(qualSpecies11_long, aes(x = reorder(species, -count), 
                             y = count, color = state, group = state)) +
  geom_line() +
  geom_point() +
  theme_pubr()
...
qualSpecies11_long$speciesOrder <- factor(qualSpecies11_long$species,
                                        levels = level_order11)


ggplot(qualSpecies11_long, aes(x = speciesOrder, 
                             y = count, color = state, group = state)) +
  geom_line() +
  geom_point() +
...
#order based on presence in bronchiectasis
qualSpecies11 <- qualSpecies11[order(-qualSpecies11$Bronchiectasis),]
level_order11 <- qualSpecies11$species
...
#olivedrab2 zwischen Micrococcus und Moraxella
#cadetblue2 zwischen Pseudomonas und Rothia
